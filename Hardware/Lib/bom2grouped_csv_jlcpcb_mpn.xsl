...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/export">
	    <xsl:text>Comment,Designator,Footprint,LCSC,Extended,MPN</xsl:text>
	    <!-- all table entries -->
	...
	<xsl:template match="components">
		...
		<xsl:value-of select="fields/field[@name='Extended']"/><xsl:text>","</xsl:text>
		<xsl:value-of select="fields/field[@name='MPN']"/><xsl:text>"</xsl:text>
	    </xsl:for-each>
	</xsl:template>
